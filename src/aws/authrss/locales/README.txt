============================
Translations For aws.authrss
============================

The updatelocales.py script rebuilds the .pot files and synchronizes the .po
files.
...
You're done. In the future, you'll need to activate that virtualenv before
running i18ndude or updatelocales.py. After you're done, you might run::
...
i18ndude cannot find all msgids. After changing Python code or templates, you
may need to run::

  $ . /where/you/want/i18ndude/bin/activate
  (i18ndude)$ cd .../aws.authrss/locales
  (i18ndude)$ python updatelocales.py

If some msgids are missing from the synched ".po" files, you may need to add
these msgids to "aws.authrss-manual.pot" (for 'aws.authrss' domain). Then re-run
the above commands.

Having aws.authrss in your language
===================================
...
Easy::
  ...
  $ cd .../aws.authrss/iw/sitestat/locales # Here ;)
  $ mkdir -p xx/LC_MESSAGES
  $ cp aws.authrss.pot xx/LC_MESSAGES/aws.authrss.po
  $ $EDITOR xx/LC_MESSAGES/aws.authrss.po
...
aws.authrss with a subversion checkout.
